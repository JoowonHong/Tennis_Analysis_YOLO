...
import constants
from utils import (
    convert_meters_to_pixels_distance,convert_pixels_distance_to_meters,
    get_foot_position,get_closest_keypoint_index,get_height_of_bbox, measure_xy_distance,
...
class MiniCourt():
    def __init__(self,frame):
        ...
        self.buffer = 50
        self.paddig_court = 20

        self.set_canvas_backgorund_box_positsion(frame)
        self.set_mini_court_postion()
    ...
    def set_court_drawing_key_points(self):
        ...
        #point 3
        drawing_key_points[6] = drawing_key_points[0] +self.court_draing_width
    ...
    def set_mini_court_postion(self):
        self.court_start_x = self.start_x + self.paddig_court
        self.court_start_y = self.start_y + self.paddig_court
        self.court_end_x = self.end_x - self.paddig_court
        self.court_end_y = self.end_y - self.paddig_court
        self.court_draing_width = self.court_end_x - self.court_start_x

        

    def set_canvas_backgorund_box_positsion(self,frame):
        frame = frame.copy()
    ...
    def draw_backgoround_rectangle(self,frame):
    ...
    def draw_mini_court(self,frames):
        output_frames = []
        for frame in frames:
            frame = self.draw_backgoround_rectangle(frame)
    ...
    def get_width_of_mini_court(self):
        return self.court_draing_width
    ...
    def get_mini_court_coordinates(self,
        ...
        distance_from_keypoint_x_meters = convert_pixels_distance_to_meters(distance_from_keypoint_x_pixels,
                                                                            player_height_in_meters,
                                                                            player_height_in_pixels
                                                                            )
        
        distance_from_keypoint_y_meters = convert_pixels_distance_to_meters(distance_from_keypoint_y_pixels,
                                                                            player_height_in_meters,
                                                                            player_height_in_pixels
                                                                            )
    ...
    def convert_bounding_boxes_to_mini_court_cooridinate2_no_use(self,player_boxes,ball_boxes,orignal_court_key_points):
        player_height = {
            1: constants.PLAYER_1_HEIGHT_METERS,
            2: constants.PLAYER_2_HEIGHT_METERS
        }
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ball_box = ball_boxes[frame_num][1]
            ball_position = get_center_of_bbox(ball_box)
            closest_plyer_id_to_ball = min(player_bbox.keys(), key=lambda x: measure_distance(ball_position,get_center_of_bbox(player_bbox[x])))
            ...
            for player_id, bbox in player_bbox.items():
                ...
                #Get the closest keypoint in pixels
                closest_key_point_index = get_closest_keypoint_index(foot_position,orignal_court_key_points,[0,2,12,13])
                closest_key_point = (orignal_court_key_points[closest_key_point_index*2],
                                     orignal_court_key_points[closest_key_point_index*2+1])

                #Get player height in pixels
                frame_index_min = max(0,frame_num-20)
                frame_index_max = min(len(player_bbox),frame_num+50)
                ...
                if closest_plyer_id_to_ball == player_id:
                    #Get the closest keypoint in pixels
                    closest_key_point_index = get_closest_keypoint_index(ball_position,orignal_court_key_points,[0,2,12,13])
                    closest_key_point = (orignal_court_key_points[closest_key_point_index*2],
                                     orignal_court_key_points[closest_key_point_index*2+1])
        ...
        return output_player_bboxes_dict,output_ball_boxes
